...
Версия 6.3 - УЛЬТРАСТАБИЛЬНЫЙ РЕЛИЗ (ИСПРАВЛЕН КОНФЛИКТ)
"""
...
# Наши модули
from config import VERSION, PORT, LOCAL_EXCEL_PATH
...
if BOT_TOKEN:
    try:
        # Сначала пробуем получить информацию о вебхуке
        ...
        # Также пробуем остановить все активные сессии polling
        requests.post(
...
async def debug_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    response = (
        ...
        f"━━━━━━━━━━━━━━━━\n"
        f"💡 Бот работает 24/7 и никогда не спит!"
    )
...
async def start_bot():
    ...
    if bot_started:
        logger.info("⚠️ Бот уже запущен, пропускаем")
        return True
    ...
    def ping_worker():
        ...
        if not render_url:
            try:
                ...
                logger.warning(f"⚠️ Использую localhost: {render_url}")
        
        logger.info(f"🌍 Авто-пинг будет использовать: {render_url}")
